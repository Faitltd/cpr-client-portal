...
- [ ] **Zoho dead-weight pass** — delete the 11 dormant rules (`June Task - Deposit Invoice`, `June Task - PDA Needed`, `Send Portal Invitation`, `PDA Signed: Send Client Portal Invitation`, `Auto Archive Closed Deals`, `Client Email - Jobsite Details`, `Deals_ZohoFlow…Precon PDF`, `Start Date Determines Material Orders…`, `Estimate review needed follow up 1/2`). Rename junk functions: `xdmvc`, `Create_Cliq_Channels_for_Deal1`, `moveWorkDriveFolderOnCompletion2`. *Must be done in Zoho UI (no delete-rule API).*
