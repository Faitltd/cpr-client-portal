...
const DEAL_FIELDS = ['Deal_Name', 'Stage', 'Contact_Name', 'Project_ID', 'Zoho_Projects_ID', 'Deal_Amount'].join(',');
const DEAL_LIMIT = 20;
...
async function computeProjectHealth(deal: any): Promise<HealthProject> {
	...
	const signals: HealthSignals = {
		schedule: await withSignalFallback(dealId, 'schedule', async () => {
			const projectId = parseZohoProjectIds(deal?.Project_ID ?? deal?.Zoho_Projects_ID)[0];
